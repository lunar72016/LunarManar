...
export default function StudioSettingsPage({ settings, loading, saving, uploading, error, onSave, onUploadAvatar }: StudioSettingsPageProps) {
  ...
  return <main className="min-h-[calc(100vh-70px)] bg-[#faf7f2] px-4 py-5 sm:px-7 sm:py-7"><div className="mx-auto max-w-6xl space-y-7"><div><p className="font-display text-3xl font-semibold text-[#294335]">丹青設案</p><p className="mt-2 text-sm text-[#88786b]">設定組合底價與倍率後，寫畫起約會自動帶入計算結果。</p></div>{error && <div className="rounded-xl border border-[#efc8ba] bg-[#fff4ef] px-4 py-3 text-sm text-[#8b4d39]">{error}</div>}
    <section className="grid gap-6 rounded-[1.5rem] border border-[#e9e0d8] bg-[#fffdfa] p-5 shadow-[0_10px_35px_rgba(83,63,43,.06)] sm:grid-cols-[auto_1fr] sm:p-6"><div className="relative h-fit"><Avatar className="h-24 w-24 border-4 border-[#e5eee5]"><AvatarImage src={draft.avatarUrl} alt="工作室頭像" /><AvatarFallback className="bg-[#dce8dd] font-display text-3xl text-[#315741]">{draft.studioName.slice(0, 1).toUpperCase() || "L"}</AvatarFallback></Avatar><label className="absolute -bottom-1 -right-1 flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-[#355b48] text-white shadow-md hover:bg-[#294a3a]"><Camera className="h-4 w-4" /><input className="sr-only" type="file" accept="image/*" onChange={selectAvatar} disabled={uploading} /></label></div><div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto]"><label className="grid gap-2 text-sm font-medium text-[#51463d]"><span>工作室名稱</span><Input value={draft.studioName} onChange={(event) => setDraft((current) => ({ ...current, studioName: event.target.value }))} placeholder="例如 Lunar Atelier" /></label><div className="flex items-end"><label className="inline-flex cursor-pointer items-center gap-2 rounded-xl border border-[#decec2] bg-[#fffdfa] px-3 py-2 text-sm text-[#66574b] hover:bg-[#f8f1ea]"><ImageUp className="h-4 w-4" />{uploading ? "上傳中…" : "更換頭像"}<input className="sr-only" type="file" accept="image/*" onChange={selectAvatar} disabled={uploading} /></label></div><p className="text-xs leading-5 text-[#9b8b7e] sm:col-span-2">頭像將上傳至 Firebase Storage，僅允許此工作室的指定 Firebase UID 存取。建議使用小於 5MB 的 JPG、PNG 或 WebP 圖片。</p></div></section>
