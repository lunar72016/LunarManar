...
function CommissionWorkspace() {
  ...
  useEffect(() => {
    ...
    void setDoc(doc(firestoreDb, "publicStudioSettings", "studio"), { lastQueuedWeek, updatedAt: Date.now() }, { merge: true }).catch(() => undefined);
  }, [lastQueuedWeek, user?.uid]);

  useEffect(() => {
    if (!commissions.length) return;
    const signature = commissions.map((commission) => `${commission.id}:${commission.updatedAt}`).sort().join("|");
    if (signature === publicProgressSyncSignature.current) return;
    publicProgressSyncSignature.current = signature;
    void Promise.all(commissions.map((commission) => intake.syncProgress(commission))).catch(() => undefined);
  }, [commissions, intake.syncProgress]);
  ...
  const heading = activeView === "dashboard" ? "運筆宮商" : activeView === "board" ? "排畫連雲" : activeView === "archive" ? "封畫入卷" : activeView === "intake" ? "墨諾函箋" : activeView === "trash" ? "落紙餘灰" : "丹青設案";
...
function TrashView({ items, error, onRestore, onDelete }: { items: import("@/hooks/useTrash").TrashItem[]; error: string | null; onRestore: (item: import("@/hooks/useTrash").TrashItem) => Promise<void>; onDelete: (item: import("@/hooks/useTrash").TrashItem) => Promise<void> }) {
  return <section className="mx-auto max-w-5xl rounded-[1.5rem] border border-[#cfd9cf] bg-[#fffdfa] p-5 shadow-[0_10px_35px_rgba(40,59,49,.06)] sm:p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="font-display text-2xl font-semibold text-[#283b31]">落紙餘灰</h2><p className="mt-1 text-sm text-[#456153]">誤置排單與待啟墨函會在此留存 7 日，可隨時枯木逢春；到期後每日灰飛煙滅。</p></div><span className="rounded-full bg-[#edf2ed] px-3 py-1 text-xs font-medium text-[#355b48]">{items.length} 項</span></div>{error && <p className="mt-4 rounded-xl border border-[#e4c7b1] bg-[#fff6f0] px-3 py-2 text-sm text-[#a9573c]">落紙餘灰同步提示：{error}</p>}{items.length ? <div className="mt-5 space-y-3">{items.map((item) => <article key={item.id} className="flex flex-col gap-3 rounded-2xl border border-[#d8ded5] bg-[#f8f9fa] p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="truncate font-medium text-[#283b31]">{item.label}</p><p className="mt-1 text-xs text-[#456153]">{item.kind === "commission" ? "排單" : "待啟墨函"} · {new Date(item.deletedAt).toLocaleString("zh-TW")} 置入 · 尚餘 {getTrashDaysRemaining(item)} 日</p></div><div className="flex shrink-0 gap-2"><Button variant="outline" className="border-[#b9cdbd] text-[#355b48] hover:bg-[#edf5ed]" onClick={() => void onRestore(item)}><RotateCcw className="mr-1.5 h-4 w-4" />枯木逢春</Button><Button variant="outline" className="border-[#e4c7b1] text-[#a9573c] hover:bg-[#fff0e9]" onClick={() => void onDelete(item)}><Trash2 className="h-4 w-4" /><span className="sr-only">灰飛煙滅</span></Button></div></article>)}</div> : <div className="py-12 text-center"><Trash2 className="mx-auto h-7 w-7 text-[#6c9575]" /><p className="mt-3 text-sm text-[#456153]">目前沒有可枯木逢春的落紙餘灰。</p></div>}</section>;
}
