...
export function CommissionDialog({ commission, open, onOpenChange, onSave, onDelete, settings, defaultScheduleWeekStart, lastQueuedWeek }: CommissionDialogProps) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="hui-dialog min-w-0 max-h-[88vh] w-full max-w-[calc(100%-1rem)] overflow-x-hidden overflow-y-scroll p-0 sm:max-w-[36rem] sm:rounded-[1.5rem]"><div className="sticky top-0 z-10 border-b border-[#d8ded5] bg-[#fffdfa]/95 px-4 pb-4 pt-6 backdrop-blur sm:px-8"><DialogHeader><DialogTitle className="font-display text-2xl text-[#283b31]">{commission ? "編輯畫約" : "寫畫起約"}</DialogTitle><DialogDescription className="text-[#456153]">先安放預計週次、作畫內容與期限；系統會依交稿日自動判斷加急。</DialogDescription></DialogHeader></div>
    <div className="min-w-0 space-y-8 px-4 py-6 sm:px-8">
      <section className="space-y-4"><SectionTitle icon={<Sparkles className="h-4 w-4" />} title="委託人與排程" /><div className="grid gap-4 md:grid-cols-2"><Field label="委託人姓名 *"><Input value={draft.clientName} onChange={(event) => update("clientName", event.target.value)} /></Field><Field label="委託單編號"><Input value={draft.orderCode} onChange={(event) => update("orderCode", event.target.value)} placeholder="未填將自動產生" /></Field><Field label="聯絡管道"><Select value={draft.contactChannel} onValueChange={(value) => update("contactChannel", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{contactChannels.map((channel) => <SelectItem key={channel} value={channel}>{channel}</SelectItem>)}</SelectContent></Select></Field><Field label="帳號／聯絡方式"><Input value={draft.contactHandle} onChange={(event) => update("contactHandle", event.target.value)} /></Field></div>
