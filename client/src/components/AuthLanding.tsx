...
import { BadgePlus, KeyRound, LogIn } from "lucide-react";
...
export function AuthLanding() {
  ...
  const googleLogin = async () => {
    ...
    try {
      ...
      setError(describeFirebaseAuthError(nextError));
    } finally {
  ...
  const lookupProgress = () => {
    ...
    if (!isPortalAccessCode(normalized)) {
      setError("請輸入完整的對契符節。");
      return;
    }
    clearGoogleSignInIssue();
  ...
  return (
    <div className="min-h-screen bg-[#fffdfa] p-5">
      <div className="mx-auto grid min-h-[calc(100vh-2.5rem)] max-w-6xl overflow-hidden rounded-[2rem] border border-[#cfd9cf] bg-[#fffdfa] shadow-[0_24px_70px_rgba(40,59,49,.13)] lg:grid-cols-[1.1fr_.9fr]">
        <aside className="relative hidden overflow-hidden bg-[#283b31] p-12 text-[#fffdfa] lg:block">
        ...
        <main className="flex items-center justify-center p-7 sm:p-12">
          <div className="w-full max-w-sm space-y-5">
            ...
            <Button className="w-full border-[#d4a359] bg-[#d4a359] text-[#1f382c] hover:bg-[#bd8a45]" variant="outline" disabled={googleLoading} onClick={() => void googleLogin()}>
              ...
              {googleLoading ? "正前往 Google…" : "使用 Google 帳號登入"}
            </Button>

            <div className="border-t border-[#d8ded5] pt-4">
              <Button type="button" variant="outline" className="w-full border-[#b9cdbd] bg-[#edf5ed] text-[#355b48] hover:bg-[#dce9dc]" onClick={openClientPortal}>
                <BadgePlus className="mr-1.5 h-4 w-4" />
                直接填寫委託
              </Button>

              <div className="mt-4 rounded-xl bg-[#edf5ed] p-3">
                <p className="text-xs font-medium text-[#355b48]">已有對契符節？</p>
                <div className="mt-2 flex gap-2">
                  <Input className="h-9 min-w-0 text-xs" value={portalCode} onChange={(event) => setPortalCode(event.target.value)} placeholder="HY-…" />
                  <Button type="button" size="sm" variant="outline" className="shrink-0 border-[#b9cdbd] text-[#355b48]" onClick={lookupProgress}>
            ...
            {(error || googleSignInIssue) && <>
              <p className="rounded-xl bg-[#fff0e9] px-3 py-2 text-xs leading-5 text-[#8e4932]">{error || googleSignInIssue}</p>
              <SafariGoogleSignInHint visible={Boolean(googleSignInIssue)} />
            </>}
          </div>
        </main>
      </div>
