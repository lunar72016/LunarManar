...
import { Check, CheckCircle2, Copy, LoaderCircle, LogIn, Plus, ShieldCheck, Trash2 } from "lucide-react";
import type { PublicScheduleChoice } from "@/pages/ClientPortalPage";
import { useState } from "react";

export type PublicSubmissionFormState = {
  clientName: string; contactEmail: string; contactChannel: string; contactHandle: string; referenceUrls: string;
  scheduleChoice: PublicScheduleChoice; reservationDate: string; artworkItems: ArtworkItem[]; licenses: LicenseOption[];
  rushDueDate: string; privacyMode: PrivacyMode; privacyUntil: string;
  characterSettingNote: string; poseNote: string; costumeDesignNote: string; accessoryNote: string; requirements: string; deliveryNote: string;
};

type PublicFormProps = {
  form: PublicSubmissionFormState; settings: StudioSettings; pricingReady: boolean; previewAmount: number; previewMultiplier: number;
  update: (key: keyof PublicSubmissionFormState, value: PublicSubmissionFormState[keyof PublicSubmissionFormState]) => void;
  onSubmit: () => Promise<void>; submitting: boolean; loading: boolean; signedInWithGoogle: boolean; accountEmail: string; queueWeekLabel: string | null;
...
export function PublicSubmissionForm({ form, settings, pricingReady, previewAmount, previewMultiplier, update, onSubmit, submitting, loading, signedInWithGoogle, accountEmail, queueWeekLabel, onGoogle, onSignOut, resultCode }: PublicFormProps) {
  ...
  const setSchedule = (choice: PublicScheduleChoice) => update("scheduleChoice", choice);
  return <section className="mt-5 rounded-[2rem] border border-[#cfd9cf] bg-[#fffdfa] p-5 shadow-[0_14px_32px_rgba(40,59,49,.06)] sm:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="font-display text-2xl">懸榜昭繪</h2><p className="mt-1 text-sm leading-6 text-[#456153]">請填寫作畫項目與設定稿網址；繪師將啟讀墨諾函箋並與您確認細節。</p></div>{signedInWithGoogle ? <div className="rounded-xl bg-[#edf5ed] px-3 py-2 text-xs text-[#355b48]"><span className="max-w-36 truncate">{accountEmail}</span><button className="ml-2 underline" onClick={onSignOut}>登出</button></div> : <Button variant="outline" className="border-[#b9cdbd] text-[#355b48]" onClick={() => void onGoogle()}><LogIn className="mr-1.5 h-4 w-4" />以 Google 帳號填寫</Button>}</div><div className="mt-6 grid gap-4 sm:grid-cols-2"><FormField label="寄墨主姓名 *"><Input value={form.clientName} onChange={(event) => update("clientName", event.target.value)} /></FormField><FormField label="聯絡管道"><Select value={form.contactChannel} onValueChange={(value) => update("contactChannel", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{contactChannels.map((channel) => <SelectItem key={channel} value={channel}>{channel}</SelectItem>)}</SelectContent></Select></FormField><FormField label="電子郵件 *"><Input type="email" value={form.contactEmail} onChange={(event) => update("contactEmail", event.target.value)} /></FormField><FormField label="帳號／聯絡方式"><Input value={form.contactHandle} onChange={(event) => update("contactHandle", event.target.value)} /></FormField></div><section className="mt-7 border-t border-[#d8ded5] pt-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-display text-xl">作畫項目 *</h3><p className="mt-1 text-xs leading-5 text-[#456153]">一張畫可填多個項目；底價會依每項人物數量、範圍與精緻度加總。</p></div><Button type="button" variant="outline" className="border-[#b9cdbd] text-[#355b48]" onClick={() => update("artworkItems", [...form.artworkItems, publicArtworkItem(settings)])}><Plus className="mr-1.5 h-4 w-4" />增加項目</Button></div>{!pricingReady && <p className="mt-4 rounded-xl bg-[#fff7f1] px-3 py-2 text-sm text-[#8b5238]">預估底價直接同步自丹青設案；目前尚未讀到已定價項目。繪師登入一次後會自動同步，無需另行填寫公開價目。</p>}<div className="mt-4 space-y-4">{form.artworkItems.map((item, index) => <PublicArtworkItem key={item.id} item={item} index={index} settings={settings} onChange={(patch) => updateItem(item.id, patch)} onRemove={() => update("artworkItems", form.artworkItems.filter((current) => current.id !== item.id))} removable={form.artworkItems.length > 1} />)}</div><div className="mt-4 rounded-2xl border border-[#b9cdbd] bg-[#edf5ed] p-4"><p className="text-xs font-semibold tracking-[.1em] text-[#456153]">預估底價</p><p className="mt-1 font-display text-2xl font-semibold text-[#283b31]">NT$ {formatCurrency(previewAmount)}</p><p className="mt-1 text-xs text-[#456153]">已按目前倍率 ×{previewMultiplier.toFixed(1)} 試算；最終報價以繪師確認內容為準。</p></div></section><section className="mt-7 border-t border-[#d8ded5] pt-6"><h3 className="font-display text-xl">期限、加急與權利</h3><div className="mt-4 grid gap-3 sm:grid-cols-3">{([{ value: "queued", title: "排單", detail: queueWeekLabel ? `將排至${queueWeekLabel}後。` : "將排至繪師目前排單週次後。" }, { value: "reservation", title: "預約", detail: "排單日期靠近時會通知。" }, { value: "rush", title: "加急", detail: "開放加急時可使用。" }] as const).map((option) => <button type="button" key={option.value} onClick={() => setSchedule(option.value)} className={`rounded-2xl border p-3 text-left transition ${form.scheduleChoice === option.value ? "border-[#355b48] bg-[#edf5ed]" : "border-[#c9d5ca] bg-[#fffdfa]"}`}><p className="text-sm font-semibold text-[#283b31]">{option.title}</p><p className="mt-1 text-xs leading-5 text-[#456153]">{option.detail}</p></button>)}</div>{form.scheduleChoice === "reservation" && <FormField label="希望預約日期（可不填）" className="mt-4"><Input type="date" value={form.reservationDate} onChange={(event) => update("reservationDate", event.target.value)} /></FormField>}{form.scheduleChoice === "rush" && <FormField label="加急交稿日 *" className="mt-4"><Input type="date" required value={form.rushDueDate} onChange={(event) => update("rushDueDate", event.target.value)} /></FormField>}<div className="mt-5 border-t border-[#d8ded5] pt-5"><p className="text-sm font-medium text-[#355b48]">權利選項</p><div className="mt-3 flex flex-wrap gap-2">{(["commercial", "promotion", "buyout"] as LicenseOption[]).map((license) => <label key={license} className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs ${form.licenses.includes(license) ? "border-[#9bb7a0] bg-[#dce9dc] text-[#283b31]" : "border-[#c9d5ca] text-[#355b48]"}`}><Checkbox checked={form.licenses.includes(license)} onCheckedChange={(checked) => toggleLicense(license, Boolean(checked))} />{licenseLabels[license]}</label>)}</div><div className="mt-4 grid gap-4 sm:grid-cols-2"><FormField label="公開設定"><Select value={form.privacyMode} onValueChange={(value) => update("privacyMode", value as PrivacyMode)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="open">可公開</SelectItem><SelectItem value="permanent">永久不可公開</SelectItem><SelectItem value="until">指定日期前不可公開</SelectItem></SelectContent></Select></FormField>{form.privacyMode === "until" && <FormField label="不可公開至"><Input type="date" value={form.privacyUntil} onChange={(event) => update("privacyUntil", event.target.value)} /></FormField>}</div></div></section><section className="mt-7 border-t border-[#d8ded5] pt-6"><FormField label="設定稿／參考資料雲端網址"><Textarea value={form.referenceUrls} onChange={(event) => update("referenceUrls", event.target.value)} placeholder="可貼上 Google Drive、Dropbox、雲端相簿等網址；多個網址可分行填寫。" /></FormField></section><div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-[#d8ded5] pt-5"><p className="max-w-md text-xs leading-5 text-[#6c7e70]"><ShieldCheck className="mr-1 inline h-3.5 w-3.5" />未使用 Google 帳號時，系統會建立受限工作階段並提供對契符節；請妥善保存。</p><Button disabled={submitting || loading || !form.artworkItems.length} className="bg-[#355b48] text-[#fffdfa] hover:bg-[#294a3a]" onClick={() => void onSubmit()}>{submitting && <LoaderCircle className="mr-1.5 h-4 w-4 animate-spin" />}{submitting ? "寄出中…" : "寄出墨諾函箋"}</Button></div>{resultCode && <ResultCodeCard code={resultCode} />}</section>;
}
...
function FormField({ label, children, className = "" }: { label: string; children: React.ReactNode; className?: string }) { return <label className={`grid min-w-0 content-start gap-2 text-sm font-medium text-[#355b48] ${className}`}><span>{label}</span>{children}</label>; }
